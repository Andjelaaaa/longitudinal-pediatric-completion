...
def perform_registration(mov, fix):
    # Placeholder for actual registration code
    # e.g., using an image processing library like SimpleITK or ANTs
    print(f"Registering {mov} to {fix}")
    # Return the result of registration
    return f"registered_{image_1}_to_{image_2}"

# Function to skull-strip an image
def skull_strip(image_path, save_path):
    # Placeholder for actual skull-stripping code
    # e.g., using an image processing library like FSL or ANTs
    print(f"Skull-stripping {image_path}")
    os.system(f'/usr/local/fsl/bin/bet {image_path} ' \
          f'{save_path}PS14_053_corrected_brain -f 0.8 -g 0')
    os.system(f'/usr/local/fsl/bin/bet {save_path} ' \
          f'{save_path} -f 0.3 -g 0')
    

def preprocess_CP(filename, path):
    ...
    trios_df = pd.DataFrame(new_rows, columns=list(data_T1w_long_subjects.columns) + ['trio_id'])
    ...
    for sub_id, group in trios_df.groupby('sub_id_bids'):
        ...
        for trio in trios:
            ...
            path_1 = trio.iloc[0]['scan_id']
            path_2 = trio.iloc[1]['scan_id']  # This is the reference
            path_3 = trio.iloc[2]['scan_id']
            ...
            scan_1 = skull_strip(path_1, save_path)
            scan_2 = skull_strip(path_2, save_path)
            scan_3 = skull_strip(path_3, save_path)

            # Save scan_2 in dedicated space (./data/CP/sub_id_bids/trio_id/)
            
            # Perform registration on scan_1 -> scan_2 and scan_3 -> scan_2
            reg_result_1_to_2 = perform_registration(scan_1, scan_2)
            reg_result_3_to_2 = perform_registration(scan_3, scan_2)

            # Save the registration results in dedicated space (./data/CP/sub_id_bids/trio_id/)
...
def load_and_preprocess_data():
    ...
    path = './data/CP'
    data = preprocess_CP(filename, path)
